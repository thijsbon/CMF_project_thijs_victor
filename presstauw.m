%% PRESSTAUW
% for prescribed pressure gradient and wall shear stress:
% same as pressvgrad, but dudzwall calculated with wall shear stress
u_old = ones(1,Nz);
        u = [0 zeros(1,Nz-2) 0];
        test = 1;
        count = 0;
        while max(test) > 0.000001;
            %only difference from pressvgrad: calculate dudzwall with
